...
class ProbabilityTests(unittest.TestCase):
    ...
    def test_Gibbs(self):
        gibbs = Gibbs_sampler(get_game_network(), [2,3,1,0,1,2])
    def test_MH(self):
        i = 0
        while i < 100:
            list = []
            list.append(numpy.random.randint(4))
            list.append(numpy.random.randint(4))
            list.append(numpy.random.randint(4))
            list.append(numpy.random.randint(3))
            list.append(numpy.random.randint(3))
            list.append(numpy.random.randint(3))
            print("INPUT: ", list)
            MH = MH_sampler(get_game_network(), list)
            print("OUTPUT: ", MH)
            print("------------------------------------------------------------")
            i+=1
    def test_sampler(self):
        output = compare_sampling(get_game_network(), [2,1,0,1,1,0])
        print(output)
        i = 1
        gibbsTotal = 0
        mhTotal = 0
        while i < 5000:
            output = compare_sampling(get_game_network(), [0,0,0,0,0,0])
            gibbsTotal += output[2]
            mhTotal += output[3]
            print(output)
            i+=1
        ...
        print("Gibbs", gibbsTotal/5000)
        print("MH", mhTotal/5000)
