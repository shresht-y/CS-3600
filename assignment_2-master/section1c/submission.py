...
def minimax(player, game, time_left, depth, my_turn=True):
    ...
    if my_turn == True:
        ...
        if depth == 0 or time_left() < 10: #must terminate at depth = 0 or when the time is up
            #need to return a null state/move
            return None, player.utility(game, True)
        best_move = None
        highest_value = -100000 #need a very small number that I know the board wont generate
        for move in player_moves:
            forecast = game.forecast_move(move) #game forecase
            forecast_board = forecast[0] #forecasted board
            forecast_win = forecast[1] #boolean for if the game is over
...
def alphabeta(player, game, time_left, depth, alpha=float("-inf"), beta=float("inf"), my_turn=True):
    ...
    if my_turn == True:
        player = game.get_active_player()
        player_moves = game.get_player_moves(player)
        if depth == 0 or time_left() < 200: #just return the best move we have if we run out of time/depth
            return None, player.utility(game, True)
